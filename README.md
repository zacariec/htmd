...
- **Partial syntax:** incomplete custom-element tags are buffered rather than flashed as literal markup. `progressive` components (messages, choice groups, code) render available content while open; `complete` components (choice items, images, files, tables, refine prompts, fragments) render nothing until their closing tag arrives, so partial content is never interactive. `data-htmd-pending` marks known incomplete syntax.
