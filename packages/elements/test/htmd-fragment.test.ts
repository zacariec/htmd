...
describe('htmd-fragment — allowed tags', () => {
  ...
  it('renders img with sanitized src and reserved dimensions', async () => {
    const fragment = await mountFragment({
      tag: 'img',
  ...
  it('renders registered custom elements', async () => {
    const fragment = await mountFragment({
      tag: 'image-card',
      attrs: { src: '/x.png', alt: 'x', width: '4', height: '3' },
    });
...
describe('htmd-fragment — rejection paths', () => {
  ...
  it('rejects unregistered custom elements', async () => {
    const fragment = await mountFragment({ tag: 'not-registered', text: 'nope' });

    expect(fragment.shadowRoot?.querySelector('not-registered')).toBeNull();
  });

  it('drops on* attributes', async () => {
    const fragment = await mountFragment({
      tag: 'image-card',
      attrs: { onerror: 'alert(1)', onclick: 'alert(2)', alt: 'ok' },
    });

    const card = fragment.shadowRoot?.querySelector('image-card');
    expect(card?.getAttribute('onerror')).toBeNull();
    expect(card?.getAttribute('onclick')).toBeNull();
    expect(card?.getAttribute('alt')).toBe('ok');
  });
